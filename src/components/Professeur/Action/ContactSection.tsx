...
const ContactSection: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTeachers = async () => {
      ...
      try {
        ...
        const q = query(usersRef, where("role", "==", "admine"));
  ...
  <span className="text-sm text-slate-400">{teacher.firstName}</span>
  <span className="text-xs text-slate-500">•</span>
  <span className="text-xs text-slate-500">
    {teacher.role === "admine" ? "Admin" : teacher.role}
  </span>
</div>
        ...
        {selectedTeacher ? (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
              {getMessagesBetweenUsers(selectedTeacher.id,sessionStorage.uid).length === 0 ? (
